...
const LogIn = () => {
  ...
  const [, , , logInUser, setLogInUser] = useContext(productContext);
  ...
  return (
    <div className="col-md-6" id="login-box">
      <span className="text-danger">{logInUser.error}</span>
      <h1>{newUser ? "Sign up" : "Log In"}</h1>
      <form onSubmit={handleSubmit(onSubmit)}>
        {newUser && <input type="text" name="username" placeholder="Full Name" ref={register({ required: true })} />}
        {errors.username && <span className="text-danger">Name is required</span>}

        <input type="email" name="email" placeholder="E-mail" ref={register({ required: true, pattern: /^\S+@\S+$/i })} />
      ...
      {!newUser && (
        <p>
          New member? <Link onClick={() => setNewUser(true)}>Register</Link> here
        </p>
      )}
      <button onClick={hendelSignInWithGoogle} className="social-signin google">
        Log in with Google+
      </button>
    </div>
  );
